...
function cacheKey(
  models: string[],
  caseCount: number,
  opts: { testCaseIndex?: string; includePerCase?: boolean },
): string {
...
async function listAvailableModels(ollamaEndpoints: string[], defaultModel: string): Promise<string[]> {
  try {
    ...
    if (!res.ok) return defaultModel ? [defaultModel] : [];
...
async function loadTestCases(index: string): Promise<TripleTestCase[]> {
  const cfg = getEsConfig();
  try {
...
async function validateOne(
  ...
  tc: TripleTestCase,
  signal?: AbortSignal,
): Promise<{ predicted: boolean | null; raw: string }> {
  try {
...
export async function tripleValidation(
  ...
  for (const model of models) {
    ...
    for (const tc of valid) {
      ...
      if (opts.includePerCase) perCase.push({ utterance: tc.utterance, triple: tc.triple, expected: true, predicted, raw });
      if (predicted === null) errors++;
      else if (predicted === true) correct++;
      else { wrong++; fn++; }
    }
    ...
    for (const tc of invalid) {
      const { predicted, raw } = await validateOne(ollamaEndpoints, model, tc, opts.signal);
      if (opts.includePerCase) perCase.push({ utterance: tc.utterance, triple: tc.triple, expected: false, predicted, raw });
      if (predicted === null) errors++;
      else if (predicted === false) correct++;
      else { wrong++; fp++; }
    }
