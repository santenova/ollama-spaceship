...
async function invokeWithAbortAndLog(client, params) {
    ...
    try {
        return await client.clientLogger.timed('InvokeLLM', () => client.integrations.Core.InvokeLLM({ ...params, signal: controller.signal }), { key, hasTools: !!params?.tools });
    }
    finally {
...
let _abortCounter = 0;
async function withAbort(client, label, fn) {
...
async function testCalculator() {
    ...
    try {
        ...
        const assistantContent = assistantMsg?.content ?? assistantMsg?.thinking ?? '';
        let finalText = '';
...
async function testFlightTracker() {
    const { emit, log } = makeRunner();
...
function _validateClientConfig(cfg) {
    const errors = [];
    if (!cfg.serverUrl)
...
function _createAuthMiddleware(opts) {
    return {
...
function _createCircuitBreaker(name, opts) {
    let failures = 0;
    let openAt = null;
    let _state = 'closed';
    return {
        ...
        canCall() {
            if (_state !== 'open')
                return true;
            if (Date.now() - openAt > opts.recoveryTimeMs) {
                _state = 'half-open';
                return true;
            }
            return false;
...
function _createBatcher(executor, windowMs) {
    let batch = [];
    let timer = null;
    const flush = async () => {
        const current = batch;
...
const _toolRegistry = (() => {
    const tools = {};
    return {
        register(name, fn) { tools[name] = fn; },
        unregister(name) { delete tools[name]; },
        call(name, ...args) { return tools[name]?.(...args); },
        has(name) { return name in tools; },
...
const _abortManager = (() => {
    const controllers = {};
    return {
        create(id) { const c = new AbortController(); controllers[id] = c; return c; },
        cancel(id) { controllers[id]?.abort(); delete controllers[id]; },
        cancelAll() { Object.keys(controllers).forEach(id => { controllers[id].abort(); delete controllers[id]; }); },
        isActive(id) { return id in controllers; },
    };
})();
const _telemetry = (() => {
    const subs = {};
...
// ── B1 Config Schema ──────────────────────────────────────────────────────────
async function testB1ConfigSchema() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (!valid.valid)
            throw new Error('Valid config reported invalid');
...
// ── B2 Auth Middleware ────────────────────────────────────────────────────────
async function testB2AuthMiddleware() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (!h.Authorization?.includes('tok-abc123'))
            throw new Error('Token not injected');
        const mw2 = _createAuthMiddleware({ getToken: () => null });
        const h2 = mw2.injectAuthHeaders({});
        if (h2.Authorization)
            throw new Error('Null token should produce no Authorization header');
        return { name, pass: true, output: log };
    }
    catch (e) {
        return { name, pass: false, output: log, error: e?.message };
...
// ── B3 Circuit Breaker ────────────────────────────────────────────────────────
async function testB3CircuitBreaker() {
    const { emit, log } = makeRunner();
...
// ── B4 Request Batcher ────────────────────────────────────────────────────────
async function testB4RequestBatcher() {
    const { emit, log } = makeRunner();
    const name = 'B4 Request Batcher';
    try {
        let execCount = 0;
        const batched = _createBatcher(async (batch) => { execCount++; return batch.map(([n]) => n * 2); }, 30);
        const [a, b, c, d] = await Promise.all([batched(1), batched(2), batched(3), batched(4)]);
        if (a !== 2 || b !== 4 || c !== 6 || d !== 8)
            throw new Error('Wrong batch results');
...
// ── B5 Tool Registry ──────────────────────────────────────────────────────────
async function testB5ToolRegistry() {
    const { emit, log } = makeRunner();
    const name = 'B5 Tool Registry';
    try {
        _toolRegistry.register('_echo', async (x) => `echo:${x}`);
        _toolRegistry.register('_double', async (n) => n * 2);
        const r1 = await _toolRegistry.call('_echo', 'hello');
        const r2 = await _toolRegistry.call('_double', 7);
        const listed = _toolRegistry.list().filter((t) => t.startsWith('_'));
...
// ── B6 Abort Manager ─────────────────────────────────────────────────────────
async function testB6AbortManager() {
    const { emit, log } = makeRunner();
...
// ── B7 Telemetry ──────────────────────────────────────────────────────────────
async function testB7Telemetry() {
    const { emit, log } = makeRunner();
...
async function testB8ModelRouter() {
    const { emit, log } = makeRunner();
...
async function testB9LocalStorageConfigMerge() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (storedModel !== 'test-model-from-ls')
            throw new Error('Model not persisted');
        if (eps[0] !== 'http://ep1')
            throw new Error('Endpoints not persisted');
        return { name, pass: true, output: log };
    }
    catch (e) {
        return { name, pass: false, output: log, error: e?.message };
...
async function testC18PromptRouter() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (!routedThinkingModel)
            throw new Error('modelRouter.resolve(thinking) returned empty');
        ...
        const enhanced = await withAbort(client, 'promptRouter.enhance', (signal) => client.promptRouter.enhance(userMessage, {
            TaskType: 'thinking',
        ...
        if (enhanced === 'marine biology report')
            throw new Error('enhance() returned raw input — enhancement did not run');
        ...
        const taskTypes = ['chat', 'thinking', 'json'];
        ...
        if (!complexFastest)
            emit('  ⚠ no model satisfies chat + tools + thinking');
        else
            emit(`  ✅ fastest chat model with tools+thinking: "${complexFastest}"`);
...
async function testC1EndpointResolution() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (!isUrl)
            throw new Error(`Ollama endpoint "${ep}" does not look like a URL`);
        ...
        if (!esIsUrl)
            throw new Error(`ES endpoint "${esEp}" does not look like a URL`);
        return { name, pass: true, output: log };
    }
    catch (e) {
        return { name, pass: false, output: log, error: e?.message };
...
async function testC3ESClusterHealth() {
    const { emit, log } = makeRunner();
...
async function testC4ESEntityIndices() {
    const { emit, log } = makeRunner();
...
async function testC5EntityIndexMap() {
    const { emit, log } = makeRunner();
    const name = 'C5 Entity Index Map Integrity';
    try {
        const { createClient, config } = await getClientModule();
        const client = createClient(config);
        const names = client.entities.map((e) => e.name);
        const indices = client.entities.map((e) => e.defaultIndex);
        const uniqueNames = new Set(names).size === names.length;
        const uniqueIndices = new Set(indices).size === indices.length;
        if (!uniqueNames)
            throw new Error('Duplicate entity names in client.entities');
        if (!uniqueIndices)
            throw new Error('Duplicate index names in client.entities');
        client.entities.forEach((e) => emit(`${e.name.padEnd(30)} → ${e.defaultIndex}`));
        return { name, pass: true, output: log };
    }
    catch (e) {
        return { name, pass: false, output: log, error: e?.message };
...
async function testC2ESConfigPersistence() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (loaded.enabled !== !original.enabled)
            throw new Error('saveEsConfig did not persist enabled toggle');
        // restore
...
async function testC6ESPersonaFetch() {
    const { emit, log } = makeRunner();
    const name = 'C6 ES Persona Fetch (createClient + client.esEntities.Persona.list)';
    try {
        const { createClient, config } = await getClientModule();
        const client = createClient(config);
        const personaEntry = client.entities.find((e) => e.name === 'Persona');
        if (!personaEntry)
            throw new Error('Persona entity not found in client.entities');
...
async function testC7ESPersonaSearch() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (r1.length === 0 && r2.length === 0)
            emit('  (no matches — index may be empty; operation itself succeeded)');
...
async function testC8ESPersonaCreate() {
    const { emit, log } = makeRunner();
...
async function testC9ESPersonaDelete() {
    const { emit, log } = makeRunner();
...
async function testC10ESPersonaUpdate() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (updated.name !== 'Updated Persona C10')
            throw new Error(`name not updated — got "${updated.name}"`);
        // verify via get
        const fetched = await client.esEntities.Persona.get(created.id);
        if (fetched.name !== 'Updated Persona C10')
            throw new Error(`get() returned stale name "${fetched.name}"`);
        // cleanup
...
async function testC11ESPersonaBulkCreate() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        results.forEach((r, i) => emit(`  [${i + 1}] id: ${r.id}  name: "${r.name}"`));
        if (results.length !== batch.length)
            throw new Error(`Expected ${batch.length} results, got ${results.length}`);
        // cleanup
        for (const r of results) {
            if (r.id)
                await client.esEntities.Persona.delete(r.id);
        }
        return { name, pass: true, output: log };
    }
    catch (e) {
...
async function testC12ESPersonaBulkUpdate() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        results.forEach((r, i) => emit(`  [${i + 1}] id: ${r.id}  name: "${r.name}"`));
        const allUpdated = results.every((r) => r.name.includes('v2'));
        if (!allUpdated)
            throw new Error('Not all names were updated to v2');
        // cleanup
...
async function testC13ESPersonaUpdateMany() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        const result = await client.esEntities.Persona.updateMany({ specialization: sentinel }, { $set: { icon: '✅' } });
        if (result.updated < 2)
            throw new Error(`Expected at least 2 updated, got ${result.updated}`);
        // cleanup
...
async function testC14ESPersonaDeleteMany() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (result.deleted < 3)
            throw new Error(`Expected at least 3 deleted, got ${result.deleted}`);
        return { name, pass: true, output: log };
    }
    catch (e) {
        return { name, pass: false, output: log, error: e?.message };
...
async function testC15ESPersonaSchema() {
    const { emit, log } = makeRunner();
...
async function testC16ESPersonaSubscribe() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        const client = createClient(config);
        const events = [];
        const unsubscribe = client.esEntities.Persona.subscribe((event) => {
            events.push(event);
        });
        ...
        if (!gotCreate)
            emit('  ⚠ no create event detected (may be a timing issue; check ES refresh interval)');
...
async function testC17PersonaSearchAndChat() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        const taskTypes = ['chat', 'thinking', 'json', 'vision'];
        ...
        const reply6 = await client.integrations.Core.InvokeLLM({
            ...
            stream: true,
            think: true,
            onToken: (delta) => { tokens6.push(delta); },
        });
        ...
        const [br1, br2, br3] = await Promise.all(batchModels.map((m, i) => client.integrations.Core.InvokeLLMBatched({
            model: m,
            system: systemPrompt,
...
async function testC19Vision() {
    ...
    try {
        ...
        if (typeof vis.send !== 'function')
            throw new Error('Core.vision.send is not a function');
        if (typeof vis.encode !== 'function')
            throw new Error('Core.vision.encode is not a function');
        ...
        if (!tagRes.ok)
            throw new Error(`Ollama unreachable at ${ep}: HTTP ${tagRes.status}`);
        ...
        if (!dataUrl.startsWith('data:image/'))
            throw new Error(`encode() did not return a data URL — got "${dataUrl.slice(0, 30)}"`);
        ...
        const analysis = await withAbort(client, 'vision.send', (signal) => vis.send(ep, visModel, bareB64, 'Briefly describe this image and its dominant color in one sentence each.', analysisSchema, 0, signal));
        if (typeof analysis !== 'object' || analysis === null)
            throw new Error(`vision.send(schema) did not return an object — got ${typeof analysis}`);
        ...
        const expandedTerms = await withAbort(client, 'expandQuery', (signal) => client.integrations.Core.expandQuery(description.slice(0, 100), signal));
        if (!Array.isArray(expandedTerms) || expandedTerms.length === 0)
            throw new Error('expandQuery returned no terms');
        ...
        if (!routedChat || !routedThinking || !routedJson)
            throw new Error('modelRouter returned empty for one of chat/thinking/json');
        ...
        const enhanced = await withAbort(client, 'promptRouter.enhance', (signal) => client.promptRouter.enhance(rawPrompt, { TaskType: 'chat', Speed: 80, defaultModel: fallbackModel, signal }));
        if (typeof enhanced !== 'string' || !enhanced)
            throw new Error('promptRouter.enhance returned empty');
        ...
        if (!thinkContent)
            throw new Error('InvokeLLM(thinking) returned empty content');
        ...
        const [br1, br2, br3] = await Promise.all(batchModels.map((m, i) => withAbort(client, 'InvokeLLMBatched', (signal) => client.integrations.Core.InvokeLLMBatched({
            model: m,
        ...
        const batchedOk = batchReplies.some(t => t.length > 0);
        if (!batchedOk)
            throw new Error('InvokeLLMBatched returned all empty');
        // ════════════════════════════════════════════════════════════════════════
        ...
        const streamResult = await invokeWithAbortAndLog(client, {
            ...
            stream: true,
            model: routedChat,
            onToken: (delta) => { streamTokens.push(delta); },
        });
        const streamText = typeof streamResult === 'string' ? streamResult : JSON.stringify(streamResult);
        if (!streamText)
            throw new Error('InvokeLLM(stream) returned empty');
        ...
        // ════════════════════════════════════════════════════════════════════════
        const pass = dataUrl.startsWith('data:image/') &&
...
export async function runTest(index) {
    const test = ALL_TESTS[index];
    if (!test)
        throw new Error(`No test at index ${index}`);
    return test();
}
export async function runSuiteA() { await runSuite('Suite A — Ollama Modules', SUITE_A); }
export async function runSuiteB() { await runSuite('Suite B — Client Infrastructure', SUITE_B); }
export async function runSuiteC() { await runSuite('Suite C — Endpoints & ES Entities', SUITE_C); }
...
async function testC20Solution() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        const result = await withAbort(client, 'Core.solution', (signal) => client.integrations.Core.solution(enhanced, signal));
        ...
        if (hasPersonas) {
            ...
            result.personas.forEach((p, i) => emit(`    [${i + 1}] ${p.name} (${p.description?.slice(0, 60) || ''})`));
        }
        if (hasManifest) {
        ...
        // Sanity checks
        if (!hasManifest)
            throw new Error('solution() returned empty or missing manifest');
...
async function testC21VisionStructured() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        emit(`  Resolved vision model: "${visModel}"`);
        const result = await withAbort(client, 'vision.send', (signal) => vis.send(ep, visModel, bareB64, 'Describe this image and its dominant color in one sentence each.', schema, 0, signal));
        const isObject = typeof result === 'object' && result !== null && !Array.isArray(result);
        if (!isObject)
            throw new Error(`send() with schema did not return a parsed object — got ${typeof result}`);
        emit(`  ── Vision send(schema) — parsed object returned ──`);
        emit(`  Keys: ${Object.keys(result).join(', ')}`);
        if (result.description)
            emit(`  description: "${String(result.description).slice(0, 80)}"`);
        if (result.dominant_color)
            emit(`  dominant_color: "${result.dominant_color}"`);
        const hasDescription = typeof result.description === 'string' && result.description.length > 0;
        if (!hasDescription)
            throw new Error('send() with schema returned no description string');
...
async function testC23Vector() {
    ...
    try {
        ...
        if (typeof vec !== 'function')
            throw new Error('Core.vector is not a function');
        ...
        const embedding = await withAbort(client, 'vector', (signal) => vec(testText, signal));
        if (!Array.isArray(embedding) || embedding.length === 0)
            throw new Error(`vector() did not return a non-empty array — got ${typeof embedding}`);
        emit(`  vector() → ${embedding.length}-dimensional vector (first 5: [${embedding.slice(0, 5).map((v) => v.toFixed(4)).join(', ')}...])`);
        ...
        if (emptyResult !== null)
            throw new Error(`vector('') should return null — got ${typeof emptyResult}`);
        emit(`  vector('') → null ✅`);
        // ════════════════════════════════════════════════════════════════════════
        // STAGE 3 — Determinism: same text → same vector
        // ════════════════════════════════════════════════════════════════════════
        emit(`\n  ── Stage 3: determinism (same text → same vector) ──`);
        const embedding2 = await withAbort(client, 'vector(determinism)', (signal) => vec(testText, signal));
        if (!Array.isArray(embedding2) || embedding2.length !== dims)
            throw new Error('Second vector() call returned different dimensions');
        let same = true;
        for (let i = 0; i < dims; i++) {
            if (Math.abs(embedding[i] - embedding2[i]) > 1e-6) {
                same = false;
                break;
            }
        }
        if (!same)
            throw new Error('vector() is not deterministic — same text produced different vectors');
        ...
        const allFinite = embedding.every((v) => typeof v === 'number' && Number.isFinite(v));
        if (!allFinite)
            throw new Error('vector() contains non-finite values (NaN/Infinity)');
        emit(`  All ${dims} values are finite numbers ✅`);
        const pass = Array.isArray(embedding) && embedding.length > 0 &&
            emptyResult === null &&
            same &&
            allFinite;
...
async function testC24StreamResponseChat() {
    const { emit, log } = makeRunner();
...
async function testC25StreamResponseVision() {
    const { emit, log } = makeRunner();
...
async function testC26StreamResponseAbort() {
    const { emit, log } = makeRunner();
...
async function testC22ClientInfraWiring() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (!ctrl || typeof ctrl.signal?.aborted !== 'boolean')
            throw new Error('abortManager.create did not return a controller with signal');
        am.cancel('c22-abort-1');
        if (!ctrl.signal.aborted)
            throw new Error('abortManager.cancel did not abort the controller signal');
        emit(`  ✅ abortManager: create → cancel → signal.aborted === true`);
        // ── 2. modelRouter ──
        const mr = client.modelRouter;
        if (!mr || typeof mr.resolve !== 'function')
            throw new Error('client.modelRouter.resolve is not a function');
        const routedChat = mr.resolve({ TaskType: 'chat', Speed: 80, defaultModel: fallbackModel });
        if (typeof routedChat !== 'string' || !routedChat)
            throw new Error(`modelRouter.resolve(chat) returned "${routedChat}"`);
        emit(`  modelRouter(chat, speed=80)  → "${routedChat}"`);
        const routedJson = mr.resolve({ TaskType: 'json', Speed: 50, defaultModel: fallbackModel });
        if (typeof routedJson !== 'string' || !routedJson)
            throw new Error(`modelRouter.resolve(json) returned "${routedJson}"`);
        ...
        if (!pr || typeof pr.enhance !== 'function')
            throw new Error('client.promptRouter.enhance is not a function');
...
async function testD1CostEstimator() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (tokens < 1)
            throw new Error(`approximateTokens returned ${tokens}`);
        ...
        if (!est.model || est.inputTokens < 1 || est.estimatedUSD < 0)
            throw new Error('estimateCost returned invalid result');
        ...
        if (finalised.outputTokens !== 200)
            throw new Error('finaliseEstimate did not set outputTokens');
        if (finalised.estimatedUSD <= est.estimatedUSD)
            throw new Error('finaliseEstimate did not increase cost with output tokens');
        ...
        if (unknown.estimatedUSD < 0)
            throw new Error('Unknown model returned negative USD');
        emit(`  estimateCost (unknown model): USD=${unknown.estimatedUSD} (fallback pricing applied)`);
        // Pricing table
        const table = getPricingTable();
        if (Object.keys(table).length === 0)
            throw new Error('getPricingTable returned empty object');
...
async function testD3EndpointFailover() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (!Array.isArray(emptyHealth) || emptyHealth.length !== 0)
            throw new Error('resetEndpointHealth did not clear health cache');
        ...
        if (!threw)
            throw new Error('withFailover should throw when all endpoints fail');
        ...
        const result = await withFailover(['http://127.0.0.2:11434', 'http://will-succeed'], async (ep) => {
            if (ep.includes('127.0.0.2'))
                throw new Error('first fails');
            return `ok:${ep}`;
        });
        if (!result.startsWith('ok:'))
            throw new Error(`withFailover did not return success from second endpoint: ${result}`);
        emit(`  withFailover (first fails, second succeeds) → "${result}"`);
        // health cache now has entries
        const health = getEndpointHealth();
        if (!Array.isArray(health))
            throw new Error('getEndpointHealth did not return array');
        emit(`  getEndpointHealth() → ${health.length} entries`);
        // pingEndpoints using real config endpoints
        const endpoints = config.ollamaEndpoints.filter((e) => !!e);
        if (endpoints.length > 0) {
            const pings = await pingEndpoints(endpoints);
            if (!Array.isArray(pings) || pings.length === 0)
                throw new Error('pingEndpoints returned empty array');
            pings.forEach((p) => emit(`  ping ${p.endpoint} → healthy=${p.healthy} latencyMs=${p.latencyMs}`));
        }
        else {
...
async function testD4ConversationMemory() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (recalled.length > 0 && ctx === null)
            throw new Error('buildMemoryContext returned null despite recalled memories');
        if (ctx)
            emit(`  buildMemoryContext → "${ctx.slice(0, 80)}..."`);
        // clearMemory
        ...
        if (afterClear.length > 0)
            emit(`  ⚠ ${afterClear.length} memories still present after clear (ES refresh delay)`);
        else
            emit(`  Verified: 0 memories after clear`);
...
async function testD5ABTesting() {
    const { emit, log } = makeRunner();
...
async function testD6ScheduledJobs() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (stillExists)
            emit(`  ⚠ job still in list after cancel (ES refresh delay)`);
        else
            emit(`  cancelJob → job removed`);
...
async function testE1OpenAIFetch() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (!ep.startsWith('http') && !ep.startsWith('/'))
            throw new Error(`resolveEndpoint returned: "${ep}"`);
        ...
        if (typeof text !== 'string' || text.length === 0)
            throw new Error(`chatCompletion returned: ${JSON.stringify(text)}`);
        ...
        if (typeof json !== 'object' || !json.word)
            throw new Error(`chatCompletion(schema) returned: ${JSON.stringify(json)}`);
        emit(`  chatCompletion(schema) → word="${json.word}"`);
        // embedText
        const vec = await embedText(ollamaEndpoints, 'nomic-embed-text', 'hello world');
        if (!Array.isArray(vec) || vec.length === 0)
            throw new Error(`embedText returned: ${JSON.stringify(vec)}`);
        emit(`  embedText → ${vec.length}-dim vector`);
        // embedText empty → null
        const nullVec = await embedText(ollamaEndpoints, 'nomic-embed-text', '');
        if (nullVec !== null)
            throw new Error(`embedText('') should return null, got ${JSON.stringify(nullVec)}`);
        emit(`  embedText('') → null ✅`);
        // cosineSimilarity
        const sim = cosineSimilarity([1, 0, 0], [1, 0, 0]);
        if (Math.abs(sim - 1) > 1e-6)
            throw new Error(`cosineSimilarity identical vectors = ${sim} (expected 1)`);
        const orth = cosineSimilarity([1, 0], [0, 1]);
        if (Math.abs(orth) > 1e-6)
            throw new Error(`cosineSimilarity orthogonal = ${orth} (expected 0)`);
...
async function testE2GroundCheckNoSources() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        if (typeof result.confidence !== 'number')
            throw new Error(`confidence not a number: ${result.confidence}`);
        if (!Array.isArray(result.flags))
            throw new Error(`flags not an array: ${result.flags}`);
        if (!Array.isArray(result.sourcesSimilarity))
            throw new Error(`sourcesSimilarity not an array`);
        ...
        if (result.confidence !== 0.5)
            emit(`  ⚠ expected 0.5 fallback confidence, got ${result.confidence}`);
...
async function testE3GroundCheckShape() {
    const { emit, log } = makeRunner();
    ...
    try {
        ...
        const result = await groundCheck('The Earth orbits the Sun and the Moon orbits the Earth.', ['nonexistent-doc-id-abc-123'], ollamaEndpoints, model);
        const validShape = typeof result.confidence === 'number' &&
            result.confidence >= 0 && result.confidence <= 1 &&
            Array.isArray(result.flags) &&
            Array.isArray(result.sourcesSimilarity);
        if (!validShape)
            throw new Error(`Invalid result shape: ${JSON.stringify(result)}`);
...
export async function runSuiteD() { await runSuite('Suite D — New Feature Modules', SUITE_D); }
export async function runSuiteE() { await runSuite('Suite E — Ground Check & openai-fetch', SUITE_E); }
