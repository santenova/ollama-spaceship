...
export function isLocalMode() {
    try {
        const prefix = globalThis.import?.meta?.env?.APP_PREFIX;
...
export const createOllamaClient = (apiKey) => {
    return { apiKey };
};
/**
...
export async function invokeLLM(opts) {
    const { prompt, messages: callerMessages, system = null, add_context_from_internet = false, response_json_schema = null, file_urls = null, model: requestedModel = null, temperature, think = false, stream = false, onToken, tools, returnRaw = false, signal, ollamaEndpoints, defaultModel, } = opts || {};
    ...
    if (add_context_from_internet) {
        try {
            ...
            if (results) {
                const contextStr = typeof results === 'string' ? results : JSON.stringify(results);
                messages.push({
                    role: 'system',
    ...
    if (!res.ok) {
        ...
        throw new Error(`InvokeLLM error: ${res.status} ${res.statusText}${errText ? ` — ${errText}` : ''}`);
    }
    // Streaming mode — delegate SSE parsing to shared helper
    if (stream && res.body) {
        let content = '';
        await parseSSEStream(res, (delta) => {
            content += delta;
            onToken?.(delta);
        });
        if (response_json_schema) {
...
export function createClient(config) {
    ...
    const authMiddleware = createAuthMiddleware({
        getToken: () => {
            try {
                return localStorage.getItem(`${LS_PREFIX}token`) || null;
            }
            catch {
                return null;
            }
    ...
    const circuitBreaker = createCircuitBreaker('primary-api', {
        ...
        onStateChange: (state) => {
            if (state === 'open')
                telemetry.emit(TelemetryEvents.CIRCUIT_OPEN, { name: 'primary-api' });
            if (state === 'closed')
                telemetry.emit(TelemetryEvents.CIRCUIT_CLOSED, { name: 'primary-api' });
        },
    });
    ...
    try {
        {
            ...
            if (resolvedServerUrl)
                ls.setItem(`${LS_PREFIX}server_url`, resolvedServerUrl);
            if (resolvedAppId)
                ls.setItem(`${LS_PREFIX}app_id`, resolvedAppId);
            if (resolvedFunctionsVersion)
                ls.setItem(`${LS_PREFIX}functions_version`, String(resolvedFunctionsVersion));
            if (resolvedModel)
                ls.setItem(`${LS_PREFIX}default_model`, resolvedModel);
            if (resolvedOllamaEndpoints?.length)
                ls.setItem('ollama_endpoints', JSON.stringify(resolvedOllamaEndpoints));
            if (resolvedHeaders)
                ls.setItem(`${LS_PREFIX}headers`, JSON.stringify(resolvedHeaders));
        }
    }
    catch (e) {
    ...
    const registerIfMissing = (name, fn) => {
        if (!toolRegistry.has?.(name))
            toolRegistry.register(name, fn);
    };
    registerIfMissing('InvokeLLM', (params) => invokeLLM({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: resolvedModel }));
    registerIfMissing('websearch', (params) => webSearch({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: resolvedModel }));
    registerIfMissing('toolbox', (params) => multiToolRun({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: resolvedModel }));
    registerIfMissing('flightTracker', (params) => flightTracker({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: resolvedModel }));
    registerIfMissing('calculator', (params) => calculator({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: resolvedModel }));
    // ── #5: LLM request batcher — allSettled so one failure doesn't cancel the whole batch ──
    const batchedInvoke = createBatcher(async (batchArgs) => {
        const settled = await Promise.allSettled(batchArgs.map(([params]) => invokeLLM({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: resolvedModel })));
    ...
    const client = { entities: [
        ...
        updateConfig: (partial) => {
            if (partial.model !== undefined) {
                ...
                try {
                    localStorage.setItem(`${LS_PREFIX}default_model`, partial.model);
                }
                catch { }
            }
            if (partial.ollamaEndpoints !== undefined) {
                ...
                try {
                    localStorage.setItem('ollama_endpoints', JSON.stringify(partial.ollamaEndpoints));
                }
                catch { }
            }
            if (partial.serverUrl !== undefined) {
                ...
                try {
                    localStorage.setItem(`${LS_PREFIX}server_url`, partial.serverUrl);
                }
                catch { }
            }
            if (partial.appId !== undefined) {
                ...
                try {
                    localStorage.setItem(`${LS_PREFIX}app_id`, partial.appId);
                }
                catch { }
            }
            if (partial.headers !== undefined) {
                ...
                try {
                    localStorage.setItem(`${LS_PREFIX}headers`, JSON.stringify(resolvedHeaders));
                }
                catch { }
            }
            if (partial.functionsVersion !== undefined) {
                ...
                try {
                    localStorage.setItem(`${LS_PREFIX}functions_version`, String(partial.functionsVersion));
                }
                catch { }
            }
        ...
        integrations: {
            Core: {
                vision: {
                     ...
                     *   );
                     */
                    async send(endpoint, model, imageBase64, prompt, schema, temperature, signal) {
                 ...
                 */
                expandQuery: (query, signal) => _expandQuery(query, resolvedOllamaEndpoints, resolvedModel, signal),
                /**
                ...
                websearch: (params) => {
                    ...
                    return clientLogger.timed('websearch', () => webSearch({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: routedModel }));
                },
                toolbox: (params) => {
                    const routedModel = modelRouter.resolve('tool_call', params?.prompt || '', resolvedModel);
                    return clientLogger.timed('toolbox', () => multiToolRun({ ...params, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: routedModel }));
                },
                InvokeLLM: (params) => {
                    ...
                    return safeExecute({
                        label: 'InvokeLLM',
                        fn: () => rateLimiter.run(() => invokeLLM({ ...params, signal: controller.signal, ollamaEndpoints: resolvedOllamaEndpoints, defaultModel: routedModel })),
                        circuitBreaker,
                ...
                async vectorIndex(params) {
                    ...
                    const result = await clientLogger.timed('vectorIndex', () => vectorPipeline({
                    ...
                    if (result?.targetIndex) {
                        const entityName = result.targetIndex
                            ...
                            .replace(/\s+/g, '');
                        const entityArr = defaultClient.entities;
                        if (Array.isArray(entityArr) && !entityArr.some((e) => e.defaultIndex === result.targetIndex)) {
                 ...
                 */
                beaming: (prompt, opts = {}) => _beaming(prompt, resolvedOllamaEndpoints, resolvedModel, opts),
                UploadFile: async () => { },
                SendEmail: async () => { },
                GenerateImage: async () => { },
        ...
        async getMessages(sessionId) {
            if (!sessionId)
                return [];
            const session = await esEntities.ChatSession.get(sessionId);
                ...
                subscribe(obs) {
                    ...
                    async function runStream() {
                        ...
                        const location = trackProgress
                            ...
                            : undefined;
                        if (trackProgress)
                            tracker = createProgressTracker(location);
                        ...
                        if (task === 'vision') {
                            ...
                            const res = await rateLimiter.run(() => trackedOllamaFetch(`${endpoint}/v1/chat/completions`, {
                                method: 'POST',
                                headers: authMiddleware.injectAuthHeaders({ 'Content-Type': 'application/json' }),
                                body: JSON.stringify(body),
                                signal,
...
export const createclientWithFallback = (_originalclient) => {
