...
describe('ClientLibrary — method contracts: stream', () => {
    ...
    test.each(['chat', 'vision', 'code', 'audio', 'thinking'])('stream accepts task type "%s"', (task) => {
...
describe('ClientLibrary — conversation memory', () => {
    test.each(['saveMemory', 'recallMemory', 'buildMemoryContext', 'clearMemory'])('%s is a function', (method) => {
        expect(typeof makeLib()[method]).toBe('function');
    });
...
describe('ClientLibrary — scheduled jobs', () => {
    test.each(['scheduleJob', 'runJob', 'runDueJobs', 'setJobStatus', 'cancelJob', 'listJobs'])('%s is a function', (method) => {
        expect(typeof makeLib()[method]).toBe('function');
    });
...
describe('ClientLibrary — endpoint failover', () => {
    test.each(['withFailover', 'pingEndpoints', 'getEndpointHealth', 'resetEndpointHealth'])('%s is a function', (method) => {
        expect(typeof makeLib()[method]).toBe('function');
    });
