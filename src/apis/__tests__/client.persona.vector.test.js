...
import { sanitizePersonaForEmbedding, PERSONA_VECTOR_MAPPING, } from '../lib/persona-embedding';
jest.setTimeout(300000); // 5 min — embedding all personas can be slow
...
async function knnSearch(index, queryVec, k = 5) {
...
describe('Persona vector index pipeline', () => {
    ...
    const skipIfDown = () => {
        if (!servicesUp)
            pending('Services unreachable — Ollama or Elasticsearch not running');
    };
    beforeAll(async () => {
        const [ollamaUp, esUp] = await Promise.all([isOllamaUp(), isEsUp()]);
        servicesUp = ollamaUp && esUp;
        if (!servicesUp)
            return; // all tests will skip individually via skipIfDown()
        ...
        marineBiologist =
            personas.find((p) => p.name?.toLowerCase().includes('marine') ||
        ...
        for (let i = 0; i < docsToIndex.length; i += BATCH) {
            ...
            for (const persona of batch) {
                ...
                if (!Array.isArray(embedding) || embedding.length === 0)
                    continue;
                const id = persona.id || `p-${i}`;
                const doc = { ...persona, embedding };
                delete doc.id;
    ...
    test('1a — closest neighbour by expertise_areas (Marine Biologist)', async () => {
        ...
        const topName = (top._source?.name || '').toLowerCase();
        const topExpertise = (top._source?.expertise_areas || []).map((e) => e.toLowerCase());
        const topTags = (top._source?.tags || []).map((t) => t.toLowerCase());
        const marineKeywords = ['marine', 'ocean', 'bio', 'eco', 'reef', 'aqua', 'sea'];
        const isRelated = marineKeywords.some((k) => topName.includes(k)) ||
